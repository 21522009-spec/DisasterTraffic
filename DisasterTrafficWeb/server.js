...
for (const [name, val] of [
    ...
    if (PLACEHOLDER_SECRETS.has(v.toLowerCase())) {
        console.error(`${name} vẫn là placeholder ('${v}'). Thay bằng chuỗi random >= 32 ký tự.`);
        process.exit(1);
    }
    if (v.length < 16) {
        console.warn(`${name} chỉ ${v.length} ký tự — yếu. Khuyến nghị >= 32 ký tự ngẫu nhiên.`);
    } else if (v.length < 32) {
        console.warn(`${name} hơi ngắn (${v.length} ký tự). Khuyến nghị >= 32 ký tự.`);
    }
}
...
if (NODE_ENV === 'production' && allowedOrigins.includes('*')) {
    console.warn('ALLOWED_ORIGINS=* trên production — mọi origin đều được phép. Nên đặt domain cụ thể.');
}
...
server.listen(PORT, () => {
    ...
    if (!TOMTOM_KEY) {
        console.warn('TOMTOM_KEY missing — traffic tile proxy và TomTom incidents sẽ không hoạt động.');
    }
});
